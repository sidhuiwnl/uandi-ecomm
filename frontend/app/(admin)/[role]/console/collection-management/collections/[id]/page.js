...
export default function CollectionProductsPage({ params: paramsPromise }) {
    ...
    const [searchQuery, setSearchQuery] = useState(""); // Added for UX: search in modal
    ...
    useEffect(() => {
        if (selectedCollection?.products) {
            // Sort by sort_order to ensure correct initial order
            const sortedProducts = [...selectedCollection.products].sort(
                (a, b) => (a.sort_order || 0) - (b.sort_order || 0)
            );
            setItems(sortedProducts);
            setSelectedProducts([]); // Reset selection when collection updates
        }
    }, [selectedCollection]);

    // Get available products (products not in current collection)
    const availableProducts = useMemo(() => {
        ...
        // Filter by search query for better UX
        if (searchQuery.trim()) {
    ...
    // Optimized sensors with activation constraint for smoother drag
    ...
    if (loading || !selectedCollection) {
        return (
            <div className="p-8 text-center text-gray-600 space-y-4">
                {/* Improved loading state with skeleton-like animation */}
                <div className="flex justify-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-rose-600"></div>
                </div>
                <p className="text-lg">Loading collection...</p>
                {/* Optional: Skeleton cards for perceived performance */}
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            {/* Header Section - More accessible and responsive */}
            ...
            {/* Product Selector Modal - Now using Headless UI for accessibility */}
                                                ...
                                                {availableProducts.map((product) => (
                                                    ...
                                                    >
                                                        <input
                                                            ...
                                                            aria-hidden="true" // Visual only, interaction via parent
                                                        />
            ...
            {/* Products Grid - Enhanced empty state and accessibility */}
            {isEmpty ? (
                ...
                >
                    <SortableContext items={itemIds} strategy={verticalListSortingStrategy}>
                        ...
                        >
                            {items.map((product) => (
                                <SortableCard
                                    key={product.product_id}
                                    id={product.product_id} // Ensure id prop for dnd-kit
                                    product={product}
                    ...
                    {/* DragOverlay - Enhanced with subtle animations */}
                    <DragOverlay>
                        {activeItem ? (
